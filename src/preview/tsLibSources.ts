...
/** The lib name compiler options resolve to when no explicit target/lib override applies. */
export const TS_DEFAULT_LIB_FILE_NAME = "lib.es2022.d.ts";

/**
 * Every `lib.*.d.ts` file transitively reachable from `compilerOptions.lib:
 * ["es2022", "dom", "dom.iterable"]` (verified against
 * `node_modules/typescript/lib/typescript.js`'s `libEntries` map and each
 * file's own `/// <reference lib="...">` directives) — the exact,
 * deliberately narrow set `tsCompilerHost.ts`'s `CompilerHost` needs to be
 * able to serve, no more. `lib.decorators*.d.ts` and
 * `lib.dom.asynciterable.d.ts` are intentionally excluded: nothing in this
 * set references them, and they're only pulled in by TS if `"decorators"`/
 * `"dom.asynciterable"` appear in `compilerOptions.lib` directly, which this
 * app's compiler options never do.
 */
export const TS_LIB_SOURCES: ReadonlyMap<string, string> = new Map([
  ["lib.es5.d.ts", libEs5],

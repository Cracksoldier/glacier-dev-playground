...
- `src/store/` — in-memory project store: `projectReducer.ts` is a plain, framework-free reducer (directly unit-testable without React), including `project/hydrate` (replace the store with persisted state, or reseed with a starter project when empty — used by "Reset local data"), `project/loadPersisted` (startup load: same as hydrate when clean, but merges persisted projects with edits made while loading instead of dropping either) and `project/markSaved` (advances `lastPersistedRevision`; `project/switch` bumps `revision` too, since the active project id is persisted); `ProjectStoreContext.tsx` wraps it in a React context/provider exposing `useProjectStore()` (active project, project list, dirty state, `saveStatus`, `persistenceNotice`, action dispatchers), composing `useProjectHydration`/`useAutosave`/`useSaveShortcut`/`useBeforeUnloadWarning` and accepting an optional injectable `repository` prop for tests. `saveStatus` reports "saving" whenever the store is dirty (including the autosave debounce window) so the leave-page warning is armed; hydration status `blocked` (newer-version or entirely unreadable data) disables autosave until an explicit reset so that data is never overwritten. `projectSelectors.ts` holds derived-state helpers. Deliberately reducer + context instead of a state-management library — only add one if it has a clear, documented benefit (see Agent operating rules below).
- `src/persistence/` — IndexedDB-backed project persistence, isolated from React. `projectRepository.ts` exposes `createIndexedDbProjectRepository()`/`createUnavailableProjectRepository()` implementing `load()`/`saveSnapshot()`/`resetAllData()`; `db.ts` wraps `idb`'s `openDB()` (schema in `schema.ts`, `DATABASE_VERSION` bumped only for object-store shape changes — separate from `PROJECT_SCHEMA_VERSION` in `src/models/project.ts`, which versions the stored record shape and is migrated per-record by `src/models/projectMigrations.ts#recoverProjectRecord`). Uses `idb` (not raw IndexedDB) for promise-based transactions, avoiding the "transaction auto-closes on unrelated awaited work" footgun; `db.ts`'s `blocking` callback closes a connection that's blocking a delete/upgrade — required so `saveSnapshot`'s delete-then-rewrite and test cleanup (`indexedDB.deleteDatabase`) don't hang. `debounce.ts` is a small framework-free debouncer used by `useAutosave`. Records read from storage are untyped (`unknown`) until `recoverProjectRecord` validates them — the one documented unchecked-cast boundary per the strict-mode rule below.
- `src/preferences/` — small, versioned localStorage-backed UI preferences, kept separate from `src/persistence/` project data (per spec §19.1). `shellPreferences.ts` (console visibility) and `workspaceLayoutPreferences.ts` (desktop workspace layout: `default`/`side`/`preview`, own key so `useActiveTab`'s wholesale save can't clobber it) never throw, falling back to defaults on any parse/shape error. Panel-size persistence instead reuses `react-resizable-panels`' own `useDefaultLayout` hook directly in `AppShell.tsx` rather than hand-rolled code, with `layoutStorage.ts`'s guarded adapter as its `storage` (never pass `window.localStorage` directly — merely reading it throws when site storage is blocked, which would crash the shell during render). Panel `defaultSize`/`minSize` must be percentage strings (`"25%"`): in react-resizable-panels v4, bare numbers mean pixels.
- `src/app/` — top-level layout: `App.tsx` is a thin delegator to `AppShell.tsx`, which composes the `Toolbar`, `PersistenceNotice` (storage-unavailable/corrupt-data warnings, with a confirmed "Reset local data" action), and the resizable workspace (via `react-resizable-panels`: exports are `Group`/`Panel`/`Separator`, not the older `PanelGroup`/`PanelResizeHandle` names; `Group` takes `orientation`, not `direction`). The workspace is an outer editors/preview `Group` wrapping an inner editors `Group`; the toolbar's workspace-layout toggle (`useWorkspaceLayout`) only flips their orientations, CSS `order` (side layout) and `display` (preview-only) — the panel tree is identical in every layout so editors and the preview iframe never remount. Panel sizes persist per arrangement and are restored imperatively via `useGroupRef().setLayout` on switch, since `defaultLayout` is only read on mount. A `Separator`'s `aria-orientation` is the opposite of its group's. `ProjectStoreProvider` wraps `AppShell`'s root, not `App`, because component tests render `<AppShell />` directly.
